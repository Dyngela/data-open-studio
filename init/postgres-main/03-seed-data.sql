...
INSERT INTO users (email, password_hash, prenom, nom, role, actif, created_at, updated_at)
SELECT * FROM (VALUES
    ('admin@opendata.io'::VARCHAR(256),
     '$2b$12$1OvsUzKzpodyoYCC4oQOk.bjxF0ADKC/7HBV8LA1doaggtU/EUokG'::TEXT,
    ...
    ('claire@opendata.io'::VARCHAR(256),
     ...
     now()::TIMESTAMPTZ, now()::TIMESTAMPTZ)
) AS seed(email, password_hash, prenom, nom, role, actif, created_at, updated_at)
