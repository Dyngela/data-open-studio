...
impl Cedrus {
    pub fn new() -> Self {
        Self {
            header: crate::cedrus::FileHeader { magic_number: *b"CEDR", version: 1 },
            schema: vec![],
            row_groups: vec![],
        }
    }
    ...
    pub fn write(&mut self, frame: &Frame) -> Result<(), String> {
        self.write_to(frame, &crate::cedrus::store_path())
    }

    /// Write a frame to an explicit directory.
    pub fn write_to(&mut self, frame: &Frame, dir: &std::path::Path) -> Result<(), String> {
        // Reset accumulated state so this instance can be reused.
        self.schema = vec![];
        self.row_groups = vec![];
    ...
    /// Read a Frame from the default store directory.
    pub fn read(name: &str) -> Result<Frame, String> {
        Self::read_from(name, &crate::cedrus::store_path())
    }

    /// Read a Frame from an explicit directory.
    pub fn read_from(name: &str, dir: &std::path::Path) -> Result<Frame, String> {
        let path = dir.join(format!("{}.cedr", name));
